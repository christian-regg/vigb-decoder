...
fn build_run_table(term: &[(u32, u32, u32)], makeup: &[(u32, u32, u32)]) -> Vec<Option<RunHit>> {
...
mod tests {
    ...
    fn white_term_run_lengths_are_sequential() {
        for (i, &(_len, _code, run)) in WHITE_TERM_ENTRIES.iter().enumerate() {
            assert_eq!(
                run, i as u32,
                "white terminating run at index {i} should be {i}"
            );
        }
    }
    ...
    fn black_term_run_lengths_are_sequential() {
        for (i, &(_len, _code, run)) in BLACK_TERM_ENTRIES.iter().enumerate() {
            assert_eq!(
                run, i as u32,
                "black terminating run at index {i} should be {i}"
            );
        }
    }
    ...
    fn extended_makeup_1792_shared() {
        ...
        let white_ext = WHITE_MAKEUP_ENTRIES
            .iter()
            .find(|&&(_, _, r)| r == 1792)
            .copied();
        let black_ext = BLACK_MAKEUP_ENTRIES
            .iter()
            .find(|&&(_, _, r)| r == 1792)
            .copied();
